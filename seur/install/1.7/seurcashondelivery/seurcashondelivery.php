...
class SeurCashOnDelivery extends PaymentModule{
    ...
    public function install()
    {
        ...
        Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'module_carrier` WHERE id_module ='.(int)$id_modulo);
    ...
    public function hookPaymentOptions($params)
    {
        ...
        if (!$this->active || !Configuration::get('SEUR2_SETTINGS_COD')) // Cash on delivery is disabled in Seur Configuration
        {
            return;
        }
        ...
        $cod_carriers = SeurCarrier::getSeurCarriersIds(true);
        ...
        if (
            !SeurLib::isInternationalShipping($country->iso_code) &&
            in_array($carrier->id_reference, $cod_carriers) &&
            $params['cart']->getOrderTotal(true, Cart::BOTH) > (float) Configuration::get('SEUR2_SETTINGS_COD_MIN') &&
            (
                (float) Configuration::get('SEUR2_SETTINGS_COD_MAX') == 0 ||
                $params['cart']->getOrderTotal(true, Cart::BOTH) < (float) Configuration::get('SEUR2_SETTINGS_COD_MAX')
            )
        ) {

            $id_seur_carrier = SeurCarrier::getSeurCarrierByIdCarrier($carrier->id);

            $seurCarrier = new SeurCarrier((int)$id_seur_carrier);

            if($seurCarrier->shipping_type!=1)
    ...
    public function getOfflinePaymentOption()
    {
        ...
        $offlineOption->setCallToActionText($this->l('SEUR Cash on delivery'))
            ->setAction($this->context->link->getModuleLink($this->name, 'validation', array(), true))
            ->setAdditionalInformation($this->context->smarty->fetch('module:seurcashondelivery/views/templates/hook/infos.tpl'));
//            ->setLogo(Media::getMediaPath(_PS_MODULE_DIR_.$this->name.'/img/logo.gif'));
    ...
    public function hookPaymentReturn($params)
    {
        ...
        $order         = $params['order'];
        $estado_actual = $order->getCurrentState();
        $euid          = '';
        $result        = '';
        ...
        return $this->context->smarty->fetch(
            ...
            .DIRECTORY_SEPARATOR.'confirmation.tpl'
        );
    ...
    public function getCargo($order, $check_id_currency = true)
    {
        ...
        if ((int)$order->id >0)
        {
            $sql = 'SELECT codfee FROM '._DB_PREFIX_.'seur2_order WHERE id_order='.(int)$order->id;
    ...
    public function getTotalPaid($order)
    {
        if ((int)$order->id >0)
        {
            $sql = 'SELECT total_paid FROM '._DB_PREFIX_.'seur2_order WHERE id_order='.(int)$order->id;
    ...
    public function updateTotalPaid($order,$total_paid)
    {
        if ((int)$order->id >0)
        {
            $sql = 'UPDATE '._DB_PREFIX_.'seur2_order SET total_paid = '.$total_paid.' WHERE id_order='.(int)$order->id;
            Db::getInstance()->execute($sql);
        }
    }
    public function calculateCartAmount($cart, $check_id_currency = true)
    {
        $minimun_Amount = Configuration::get('SEUR2_SETTINGS_COD_FEE_MIN');
...
//                    $cargo_rate = Tools::ps_round(($cart_Amount - $cargo_neto),2);
                ...
                if (!$order_creation_failed & isset($order->id))
                {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
                    {
                        ...
                        $data = array(
                            ...
                            '{delivery_phone}' => SeurLib::cleanPhone($delivery->phone ? $delivery->phone : $delivery->phone_mobile),
                            ...
                            '{invoice_phone}' => SeurLib::cleanPhone($invoice->phone ? $invoice->phone : $invoice->phone_mobile),
            ...
            DB::getInstance()->update('seur2_order',array('codfee'=>$cart_Amount),'id_order ='.$order->id);
