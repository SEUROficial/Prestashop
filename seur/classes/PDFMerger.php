...
class PDFMerger
{
	...
	{
        $ruta = _PS_VENDOR_DIR_;
        $folder = 'tecnickcom/';
        if (version_compare(_PS_VERSION_, '1.7', '<')) {
            $folder = '';
            if (!defined('_PS_VENDOR_DIR_')) {
                $ruta = _PS_MODULE_DIR_.'../';
                $folder = 'tools/';
            }
        }

        $tcpdfPath = $ruta.$folder.'tcpdf/tcpdf.php';
        if (!file_exists($tcpdfPath)) {
            echo 'File not exists: '.$tcpdfPath;
            SeurLib::log('File not exists: '.$tcpdfPath);
            die;
        }
        require_once($tcpdfPath);
        require_once(_PS_MODULE_DIR_.'seur/classes/tcpdf/tcpdi.php');
	}
